...
**الإصدار:** الـ Worker `v1.1.0` (`WORKER_VERSION` في `index.js`)
...
| `search_sku` | بحث في متغيّرات المنتجات بـ SKU أو رقم باركود — **من غير أوردر**. ⚠️ **بلا مستهلك من v1.10.0 بتاعة الهب** | GET |
...
- 🔴 **`barcode.min` في الهب بقى `1.1.0`** (هب v1.10.0). يعني نشر v1.1.0
  **شرط** مش تحسين: من غيره **ماسح باركود شوبيفاي بيفشل على كل مسح**
  بـ 400 «رقم الأوردر مطلوب» — الصفحة بتبعت `id=` والنسخة القديمة
  مابتعرفهوش. باقي المداخل (بوسطة · اليدوي) شغّالة على الاسم عادي، يعني
  **التدهور جزئي ومُعلَن** بحارس النسخة مش صامت.
- 🟡 **`search_sku` بقى بلا مستهلك** — مربع البحث المباشر اتشال من
  `sku-barcode.html` في هب v1.10.0 (قرار أحمد 07-09-2026). الـ action
  متساب زي ما هو: قراءة بحتة ومش ضار، والشيل بيحتاج قرار — هل هيرجع
  كمدخل احتياطي لإعادة طباعة ليبل ضاع من غير أوردر ولا لأ.
...
آخر مطابقة: 07-09-2026 · `index.js` v1.1.0
🔴 معلّقة: — لا شيء

---

آخر تحديث: 07-09-2026 — v1.1.0
